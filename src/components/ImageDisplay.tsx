...
import { darkMode, defaultMode, lightMode } from "../utils/cloudinary";
...
export const ImageDisplay: React.FC<{ publicId: string }> = ({ publicId }) => {
  ...
  const baseImage = defaultMode(publicId);
  const filteredImage = currentMode === 'light' ? lightMode(publicId) : darkMode(publicId);

  useEffect(() => {
    setIsLoading(true);
  }, [publicId, currentMode]);
    ...
    <div className="aspect-square w-full">
      {
        isLoading ?
          <div className="relative">
            <div className="absolute flex items-center justify-center inset-0 size-full bg-white bg-opacity-70 dark:bg-gray-800 dark:bg-opacity-70">
              <div role="status">
